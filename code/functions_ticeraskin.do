...
program PreProcessState
    ...
    * Create outcome vars
    forvalues h = -10/20 {
        ...
        loc hzn = abs(`h')
        if `h' < 0 {
            gen rgdp_L`hzn'    = L`hzn'.rgdp / L1.rgdp
            gen patents_L`hzn' = L`hzn'.patent_count  - L1.patent_count
        }
        else {
            gen rgdp_F`hzn'    = F`hzn'.rgdp / L1.rgdp
            gen patents_F`hzn' = F`hzn'.patent_count ) - asinh(L1.patent_count
        }
    }
end
    ...
    * Changes for left hand side
    forvalues h = -5/18 {
        ...
        if `h' < 0 {
            ...
            gen rgdp_L`hzn' = l`hzn'.rgdp / l.rgdp
            gen patents_L`hzn' = asinh(l`hzn'.patent_count) - asinh(l.patent_count)

        }
        else {

            gen rgdp_H`hzn' = f`hzn'.rgdp / l.rgdp
            gen patents_H`hzn' = asinh(f`hzn'.patent_count) - asinh(l.patent_count)
        }
    }
	    
* Constructing the shift share
sort county_num year

** National totals of each variable
bys year: egen nat_basic_total   = total(basic_wide_real)
bys year: egen nat_applied_total = total(applied_wide_real)

** County spending share
gen basic_county_share   = basic_wide_real   / nat_basic_total
gen applied_county_share = applied_wide_real / nat_applied_total

** Average spending shares
bys county_num: egen avg_basic_county_share   = mean(basic_county_share)
bys county_num: egen avg_applied_county_share = mean(applied_county_share)

** Bartik instrument using mean shares
sort county_num year
gen z_basic_shiftshare   = avg_basic_county_share   * (nat_basic_total)
gen z_applied_shiftshare = avg_applied_county_share * (nat_applied_total)
gen basic_ss = z_basic_shiftshare/L.rgdp
gen applied_ss = z_applied_shiftshare/L.rgdp

** Other controls and stuff
gen gdp_control = D.rgdp/L.rgdp
gen patent_control = asinh(patent_count) - asinh(l.patent_count)
